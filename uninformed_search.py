...
# graphs using Data
# using a dataset from Facebook where the 'friends lists' are 'circles', node features == profiles, ego networks, circles certain features have been anonymized
...
# build networkx graph from a Pandas DataFrame: the following graph object has been commented because it takes a while to run with this many edges and nodes
...
# finding paths and connections: build a disconnected networkx Graph object
graph_3_dict = {
    'A': ['B', 'C'],
    'B': ['A', 'C'],
    'C': ['A', 'B'],
...
# build graph from the df routes using "nx.from_pandas_edgelist". each row == edge between the "source" and "dest" columns
...
# 1. finding all shortest paths between airports in ALB and SFO. 
# 2. finding the length of any of the shortest paths from ALB to SFO 
# 3. finding the number of shortest paths from ALB to SFO
...
# edges are directed since they connect the parent nodes to their respective left and right children. value is any data or object 
# and does not need to be unique to the node. the children (left and right) themselves can be trees
class Tree:
    """Simple Binary Tree"""
...
# function prints the tree value attribute of the node, then recursively traversing the left and right branches and prints their branches
